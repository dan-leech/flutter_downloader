...
class FlutterDownloader {
  ...
  /// * `debug`: sets `true` to show console debug output
  /// * `showAllCompletedNotification`: sets `true` to show a notification when all
  /// tasks are completed when app in background (todo: add to iOS)
  static Future<Null> initialize(
      {bool debug = true, showAllCompletedNotification = true}) async {
    assert(!_initialized,
    ...
    await _channel.invokeMethod('initialize', <dynamic>[
      ...
      debug ? 1 : 0,
      showAllCompletedNotification ? 1 : 0
    ]);
    _initialized = true;
    return null;
  }
  ...
  static Future<String> enqueue(
    ...
    try {
      String taskId = await _channel.invokeMethod('enqueue', {
        ...
        'show_file_progress_notification': showNotification,
